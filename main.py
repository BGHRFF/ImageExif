...
def main():
    ...
    if option in ["1", "01"]:
        ...
        if not exif_data:
            print(f"{Colors.RED___}Burada Exif datası tapılmadı '{Colors.YELLOW}{image_path}{Colors.RED___}'")
            exit()
        _csv_file = "exif_data.csv"
        if os.path.exists(_csv_file):
            print(f"{Colors.GREEN_}Köhnə silinir '{Colors.YELLOW}{_csv_file}{Colors.GREEN_}' ...")
            os.remove(_csv_file)
        with open(_csv_file, "a", newline="") as f:
            csv_writer = csv.writer(f)
            for i in exif_data.items():
                csv_writer.writerow(i)
        print(f"{Colors.GREEN_}Data saxlanıldı: '{Colors.YELLOW}{_csv_file}{Colors.GREEN_}' ...")
    elif option in ["2", "02"]:
